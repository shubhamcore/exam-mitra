...
f"LANGUAGE INSTRUCTION: If language is 'hi' (Hindi), write EVERYTHING in pure Hindi using Devanagari script (हिन्दी) — headings, concepts, explanations, mistakes, summaries, flashcards, MCQ questions AND options, explanations — ALL must be in Hindi script. If language is 'hinglish', write in conversational Roman-script Hinglish (mix of Hindi words in English letters + English technical terms where natural, e.g. 'Projectile motion ka maximum height formula H = u²sin²θ/(2g) hota hai'). If language is 'en', write in English.\n\n"
f"Generate notes, 6-9 flashcards, and 4 MCQs for EACH of these {len(batch)} chapters:\n"
f"{batch_text}\n\n"
...
f"✓ Each note has 8-12 key concepts, 8-18 formulas/definitions, 3-5 common mistakes, 3-4 sentence summary\n"
...
f"✓ EVERY math symbol, variable, formula, subscript, superscript, Greek letter MUST be inside $...$ LaTeX delimiters — even when inside a sentence. Example: 'The acceleration $a$ is given by $F = ma$ where $m$ is mass.' NEVER leave a bare variable or formula outside $...$.\n"
f"✓ For physics/geometry/mechanism topics you MAY include 1 ASCII diagram in a key concept or summary using plain text inside a code block (triple backticks) when it genuinely helps — e.g. pulley setups, projectile paths, circuit diagrams, flowcharts for polity/economy. Keep diagrams ≤ 8 lines.\n\n"
